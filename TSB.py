...
beijing_lat = np.where(lat == 40.5)[0][0]
beijing_lon = np.where(lon == 115)[0][0]
taipei_lat = np.where(lat == 25)[0][0]
taipei_lon = np.where(lon == 121)[0][0]
shanghai_lat = np.where(lat == 31.5)[0][0]
shanghai_lon = np.where(lon == 120.5)[0][0]

#establish arrays for x-axis and y-axis
tmp_beijing = dat['t'][299:359, ilev, beijing_lat, beijing_lon]
tmp_shanghai = dat['t'][299:359, ilev, shanghai_lat, shanghai_lon]
...
#plot 4 lines
ax.plot(time_array, tmp_beijing, color='mediumblue')
ax.plot(time_array, tmp_shanghai, color='darkgreen')
ax.plot(time_array, tmp_taipei, color='#fabd00')
...
#set legend
ax.legend(['Beijing', 'Sanghai', 'Taipei'])
